...
const projectFileWithContent = (file, transpileOptions) => {
...
export const getAllProjectFiles = async (rootDir = path.join('.', '/')) => {
    try {
        ...
        return getContentOfProjectFiles(files).map((file) => {
...
const getContentOfProjectFiles = (files) => {
...
const getAppsScriptFilesFromProjectFiles = (files, rootDir) => getContentOfProjectFiles(files).map((file) => {
 ...
 */
export const hasProject = () => fs.existsSync(project.resolve());
/**
...
export const pushFiles = async (silent = false) => {
    ...
    if (scriptId) {
        ...
        if (toPush.length > 0) {
            ...
            const files = getAppsScriptFilesFromProjectFiles(orderedFiles, rootDir !== null && rootDir !== void 0 ? rootDir : path.join('.', '/'));
