...
const { addSyntheticTrailingComment, createBinary, createIdentifier, createNotEmittedStatement, createObjectLiteral, createPropertyAssignment, createToken, createVariableDeclaration, createVariableDeclarationList, createVariableStatement, EmitFlags, idText, isBinaryExpression, isEnumDeclaration, isExportDeclaration, isExpressionStatement, isIdentifier, isImportDeclaration, isImportEqualsDeclaration, isPropertyAccessExpression, ModuleKind, NodeFlags, ScriptTarget, setEmitFlags, SyntaxKind: tsSyntaxKind, transpileModule, updateSourceFileNode, version, visitEachChild, visitNode, } = typescript;
// Type guards helpers
...
const ts2gas = (source, transpileOptions = {}) => {
     ...
     */
    const exportsDefaultNodeFilter = (node) => isExpressionStatement(node) &&
     ...
     */
    const exportEsModuleNodeFilter = (node) => isExpressionStatement(node) &&
        node.pos === -1 &&
     ...
     */
    const exportFromNodeFilter = (node) => isExportDeclaration(node) && node.getChildren().some((node) => node.kind === tsSyntaxKind.FromKeyword);
    /**
     * Filter any import declaration
     */
    const importNodeFilter = (node) => isImportEqualsDeclaration(node) || isImportDeclaration(node);
    /**
     * Filter any identifier
     */
    const identifierFilter = (node) => isIdentifier(node);
    // Transformers
    /**
    ...
    const createCommentedStatement = (node) => {
        ...
        addSyntheticTrailingComment(ignoredNode, tsSyntaxKind.SingleLineCommentTrivia, node.getText().replace(/\n/g, '\\n'));
        return ignoredNode;
    };
    ...
    const ignoreNodeBeforeBuilder = (nodeFilter) => (context) => {
        const visitor = (node) => nodeFilter(node) ? createCommentedStatement(node) : visitEachChild(node, visitor, context);
    ...
    const addDummyModuleNodes = () => (sourceFile) => addDummyModule
        ? updateSourceFileNode(sourceFile, [
            createVariableStatement(undefined, createVariableDeclarationList([
                createVariableDeclaration(createIdentifier('exports'), undefined, createBinary(createIdentifier('exports'), createToken(tsSyntaxKind.BarBarToken), createObjectLiteral([], false))),
            ], NodeFlags.None)),
            createVariableStatement(undefined, createVariableDeclarationList([
                createVariableDeclaration(createIdentifier('module'), undefined, createBinary(createIdentifier('module'), createToken(tsSyntaxKind.BarBarToken), createObjectLiteral([createPropertyAssignment(createIdentifier('exports'), createIdentifier('exports'))], false))),
            ], NodeFlags.None)),
    ...
    transpileOptions = deepAssign({}, defaults, // Default (override-able)
